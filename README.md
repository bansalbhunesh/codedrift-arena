...
After each `env.step()`, **`info["metric_strip"]`** is a one-line summary (`reward`, `recall`, `verdict`, `malformed_issues`, **`grounded_in_diff`**). **`info["judge_emoji"]`** / **`info["judge_summary"]`** are **visual + plain-English** hints. **`info["judge_why_matters"]`** ties the outcome to **production impact** (e.g. “would prevent a production bug…”). **`info["confidence_strip"]`** is a short **HIGH / MEDIUM / LOW** line from diff grounding + recall (still diagnostic). Gradio Status shows emoji, strip, summary, **💡 why**, then confidence. **`info["diff_grounding"]`** lists, per ground-truth stale ref, whether that artifact’s text appears in the PR diff — **diagnostic only** (reward still uses **`ISSUES:`** only); use it to spot token-stuffing in ISSUES that is not reflected in the diff.
